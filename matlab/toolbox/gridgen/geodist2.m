function range =  geodist2 ( lat1, lon1, lat_n, lon_n )
% GEODIST2:  returns distance from lat/lon point to vector
%
% USAGE:  range = geodist2 ( lat1, lon1, latn, lonn );
%
% PARAMETERS:  
%   range:  distance in meters
%   lat1:  single latitude point
%   lon1:  single longitude point
%   latn:  vector or matrix of latitude
%   lonn:  vector or matrix of longitude
%
% The routine fakes out the "dist.m" routine written by
% the whoi brainiacs.  That routine returns distance between
% successive lat lon points.  What we want is distance from
% ONE point to a series of other points.  So we interleave
% that first point between each of the other points and
% use "dist.m" on that.  Then we return every other distance.
% Clever, eh?
...
inds = [1:1:length(lat_n)]';

%
% construct the latitude vector to feed into dist
alllatpts(2*inds) = lat_n(inds);
alllonpts(2*inds) = lon_n(inds);

alllatpts(2*inds-1) = lat1 * ones(size(inds));
alllonpts(2*inds-1) = lon1 * ones(size(inds));

range = dist ( alllatpts, alllonpts );


%
% Now, every other value is what we are looking for.
dist_inds = [1:2:num_pts*2];
range = range(dist_inds);




%
% Now, if the input was a matrix, reshape it such
% that the output is also a matrix.
range = reshape(range,r,c);
