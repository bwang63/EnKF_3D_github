function values = getcdf_b(file, varid, corner, end_point, ...
                  stride, order, change_miss, new_miss)
% GETCDF_BATCH returns a hyperslab of values from a netCDF variable
%
%  function values = getcdf_b(file, varid, corner, end_point,
%                    stride, order, change_miss, new_miss)
%
% DESCRIPTION:
% getcdf_b is an non-interactive function that gets a hyperslab
% of values from a netCDF variable; its arguments specify what data
% points are to be retrieved.  Thus, getcdf_b is suitable for use
...
%    is to be returned then order == 1 causes it to be a row vector and
%    order == 2 causes it to be a column vector.
...
%
% NOTE:
% If getcdf_b is only passed the first 4 input arguments then
% defaults are used for the other 4.  These give a stride of 1 in all
% directions, order = 1, and change_miss = 2 (i.e., missing values are
% changed to NaNs).
% If getcdf_b is only passed the first 2 input arguments then the
% default values of corner and end_point will return all of the
% values in all directions.
%
% EXAMPLE:
%  us = getcdf_b('uvd', 's-u', [1 1 1], [720 1 151], [2 1 3], 2, 3, 1000);
...
%     Date     $Date: 1997/05/23 04:56:08 $
%     RCSfile  $RCSfile: getcdf_batch.m,v $
% @(#)getcdf_b.m   1.8   92/08/05
% 
%--------------------------------------------------------------------
...
% Check that there are the correct number of arguments.  If there are 7
% arguments instead of 8 assume that the stride has been ommitted and do
% a fill with default values of 1.  If there are 2 or 4 arguments then
% set the last 3 arguments to their default values.  (The values of the
% other unknown arguments are set later when we find out the dimensions
% of the variable.)
...
if (nargin == 2) | (nargin == 4)
  ...
  new_miss = 0;
elseif nargin == 7
  new_miss = change_miss;
  change_miss = order;
  order = stride;
  stride = ones(size(end_point));
elseif nargin ~= 8
...
if varid < 0 | varid >= nvars
   error([ 'getcdf_b was passed varid = ' int2str(varid) ])
end

if all ( order ~= [ 1 2 ] )
   error([ 'getcdf_b was passed order = ' int2str(order) ])
end

if all ( change_miss ~= [ 1 2 3 ] )
   error([ 'getcdf_b was passed change_miss = ' int2str(change_miss) ])
end
...
%If there were 2 input arguments then set the values of corner, edge and
%stride to their default values.  If there were 4 input arguments then
%set the value of stride to it default value.  Otherwise, check that the
%last 6 input arguments are acceptable.  Also set take_stride which
%specifies whether strides need to be taken.

if nargin == 2
  corner = -1*ones(nvdims, 1);
...
elseif nargin == 4
  ...
  take_stride = 0;
  for i = 1:corner_max
    if (corner(i) > 0) & (end_point(i) > 0) & (stride(i) > 1)
      take_stride = 1;
    end
  end
...
% Find out whether to return a scalar, vector or matrix
...
for i = 1:nvdims
  ...
  if corner(i) < 0
    ...
    if corner(i) >= sizem | end_point(i) < 0 | end_point(i) >= sizem
      s = [ 'getcdf_b was passed corner = ' int2str(corner(i)+1) ...
...
if len_index_miss > 0
   ...
   elseif change_miss == 3
     ...
     if vartypv == nc_char
      ...
      s = [ 'getcdf_b was passed change_miss = ' int2str(change_miss) ];
      error(s)
   end
end
