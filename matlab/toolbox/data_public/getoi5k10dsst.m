function [data,lon,lat,date_used] = getoi5k10dsst(limits,date_requested,...
   varopt,deci,data_dir)
% Retrieves 10-day averaged SST closest to the date requested, within the 
% lat/lon LIMITS requested, from Alison Walker's analysis of ERIN AVHRR
%
% [data,lon,lat,date_used] = getoi5k10dsst(limits,date_requested,varopt,...
% deci,data_dir)
%---------------------------------------------------------------------
%
% Required:
...
if nargin > 2
  if ~(strcmp(varopt,'sst') | strcmp(varopt,'err'))
