function [f]=nc_read(fname,vname,tindex,grd);
%
% [f]=nc_read(fname,vname,tindex,grd);
%
% John Wilkin's hacked version of Hernan's function for reading ROMS
% variables that accounts for the water-points-only format
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (water),
  try 
    % this is how Hernan does it
    [Vnames,nvars]=nc_vname(fname);
    got_mask=strmatch(msknam,Vnames);
    if (got_mask),
      mask=ncread(fname,msknam);
    else,
      [fn,pth]=uigetfile(grid_file,'Enter ROMS grid NetCDF file...');
      gname=[pth,fn];
      mask=ncread(gname,msknam);
    end,
  catch
    % this will work if structure grd is passed through the input list
