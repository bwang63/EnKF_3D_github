...
% -----Notes:-----
% This function is similar to f_npManova2, but is for data NO REPLICATION
% or NESTED factors. With no replication, there is no SS.residual term
% so the interaction term is used instead. As a consequence you cannot test for
% an interaction. Use 'Tukey's test for non-additivity' to test for interactions.
...
for k = 1:iter
	...
	if (rep>0) % WITH REPLICATION (factor 2 is nested, so no interaction)
		...
		% interaction term:
		SS(3).among = sum(diag(H{3}*Gvar*H{3}));               
