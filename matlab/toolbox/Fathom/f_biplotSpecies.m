function [biplot,Rsq] = f_biplotSpecies(crds,species,special,iter,scale,offset,sLabels);
% - create species vectors for ordination distance biplot
%
% Usage: [biplot,rsq] = f_biplotSpecies(crds,species,{special},{iter},{scale},{offset},{sLabels});
%
% ----- Input: -----
...
%
% special = type of correlation (0 = Pearson's [default], 1 = Spearman's)
%
...
if (nargin < 3), special = 0; end; % set default correlation to Pearson's
...
for i = 1:nloop
   ...
   [xR,xP] = f_corr(thisSpecies,x,special,iter);  % correlation with 1st axis
   [yR,yP] = f_corr(thisSpecies,y,special,iter);  % correlation with 2nd axis
