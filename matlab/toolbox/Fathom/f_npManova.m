function result = f_npManova(yDis,x,type,rank,iter,verb)
% - nonparametric (permutation-based) MANOVA for ANY distance matrix
%
% USAGE: result = f_npManova(yDis,x,type,rank,iter,verb)
%
...
%        (column-wise) or a matrix of explanatory variables
% type = x specifies group membership/factor levels (default = 0)
%        x is matrix of explanatory variables               (= 1)
%
...
%
% See also: f_npManovaPW, f_anosim, f_anosim2
...
%  multivariate analysis of variance (for any two-factor ANOVA design) using permutation
%  tests. Dept. of Statistics, University of Auckland.
...
% -----Notes:-----
% Special care must be taken when coding levels of nested factors, i.e. treatment
% levels of a nested factor must not be REPEATED across different levels of the main
% factor. For example:
%
...
%
% This program takes a regression-approach to ANOVA using the General Linear Model
% (GLM) and constructs F-ratios using the UNRESTRICTED form of the model. The
% F-ratios used for each type of test are provided in the Appendix of the User's
% Manual. Some of these differ somewhat from those used in textbook examples,
% especially for balanced, mixed-model designs, but are the same as those used in
% most computer programs that use GLM (e.g., SAS and MINITAB).

% by Dave Jones,<djones@rsmas.miami.edu> May-2002
...
if (nargin < 3), type  = 0; end; % set default method to groups/factors
if (nargin < 4), rank  = 0; end; % don't rank distances by default
if (nargin < 5), iter  = 0; end; % default iterations for permutation test
...
if (type==0) %%--MANOVA--%%
	...
	case 1; % 1-Way MANOVA:
		...
		[null1,null2,null3,null4,null5,result] = f_npManova1(n,m,H,I,G,iter);
	...
	% don't create design matrix, use matrix of explanatory variables   
	error('Explanatory variables not yet implemented');
end

% -----Send output to display:-----
if ((verb>0) & (type==0))            % MANOVA:
