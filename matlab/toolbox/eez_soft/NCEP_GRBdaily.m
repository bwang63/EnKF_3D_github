function NCEP_GRBdaily(area,date1,daten,time_interval,dir_in,dir_out,file_out,refyear,areaname,others)
% NCEP_GRBdaily: Extract time,lat,lon cube from NCEP GRIB-format 
% global-coverage year-files and write to Net-CDF file.
%
% NCEP_GRBextr(area,date1,daten,time_interval,dir_in,dir_out,file_out,refyear,areaname,others)
%
...
%
%    some modes by John Wilkin May 1, 2000
%
%   Usage example:
%    area=[105 160 -45 -5];
%    areaname='Australia'; % appears in title of file
%    date1=[1987 1 1 0 0 0];
%    daten=[1996 12 31 0 0 0];
%    time_interval = 1; % subsampling interval: use 1 to get daily data, 2 for every 2nd, etc.
%       (To do: enable time-averaging)
%    dir_in='/home/eez_data/winds/ncep/GRB/daily/'; % the grib files off the CDs
%       % with the year added to the name of each file, mv uflx.sfc 1993uflx.sfc
%    refyear = 1980; % the time vector output to the .nc file will be days from the beginning
%                      of refyear.
%
% NCEP_GRBextr(area,date1,daten,time_interval,dir_in,dir_out,file_out,refyear,areaname,others)
...
f.history = ['extraction from grib files using ' mfilename ' - ' datestr(now)]
...
for irec=1:nrecs
  ...
  if (irec==1)
    ...
    % [landmask] = get_ncep('/home/eez_data/winds/ncep/GRB/fixed/land.sfc',1); 
    % f{'land'}(:,:) = permute(landmask(lonsout,latsout),[2,1]);
    f{'land'}(:,:) = zeros(size(f{'lon'}));
