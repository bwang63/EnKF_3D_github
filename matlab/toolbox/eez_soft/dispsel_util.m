...
switch action
  ...
  case 'init'
    ...
    cru_list = {};
    tot_cru = sort_nd((DATAves(DISPii)*10000)+DATAcru(DISPii));
    for ii=1:length(tot_cru)
      vvv = floor(tot_cru(ii)/10000);
      ccc = tot_cru(ii)-(vvv*10000);
  ...
  case 'one_cru'
    ...
    if ~isempty(tot_cru) & this_cru <= length(tot_cru)
      vvv = floor(tot_cru(this_cru)/10000);
      ccc = tot_cru(this_cru)-(vvv*10000);
