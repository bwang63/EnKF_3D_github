...
! get address of input      arrays
   ...
   ! set output matrix according to sizes of Sn (3)
	...
	PtIns = n_in
     ...
     c                   PtIn,PtOut,PtZin,PtZout,dataout,
     c                   errout,a,b,PtIns,pmap)
	implicit none
	integer PtIn,PtOut, ii, jj,i,j,PtIns,PtZin,PtZout,Isel,iz
      integer indexes(PtIns),PtIn1,PtOut1	
	real*8 a,b, pmap(PtOut,PtIns)
	real*8 x(PtIn),      y(PtIn),        z(PtIn,PtZin),
     c       xhat(PtOut),   yhat(PtOut),   zhat(PtOut,PtZout),
     C       dataout(PtOut,PtZout),errout(PtOut),datain(PtIn,PtZin),
     c     dataout_tmp(1,PtZout),zrOut(PtOut,PtZout),zrIn(PtIn,PtZin)
	real*8 distb, dista, dist_ref,dist
 
      real*8 xs(PtIns),ys(PtIns),zs(PtZin),out_y(PtZout),
     c          datainsZ(PtZin),datains(PtIns,PtZout),
     c          zhat_tmp(PtZout)
      real*8  datain1(PtIns),x1(PtIns),y1(PtIns),
     c          dataout1(PtIns),xhat1(PtIns),yhat1(PtIns),
     c          errout1(PtIns)
c init to zero matricies      
	do i=1, PtOut
	 ...
	 print *, 'PtIns = ', PtIns
	 print *, '  need to building map '
       return
      endif	

      print*, PtIn,PtOut,PtZin,PtZout, PtIns
c do the objective map
	do i=1,PtOut             !- LOOP 1 PtOut
...
c     vertical linear interpolation
	  ...
	  enddo
!-------------------------------------------------------------
! take care of the horizontal mapping for the deeper points
!
!        goto 41
	  do iz=1,PtZout
	    PtIn1=0
	    PtOut1=0
	      do ii=1,PtIns
	        if (datains(ii, iz ).ne.9999) then
		     PtIn1=PtIn1 + 1
		     datain1(PtIn1) = datains(ii, iz )
		     x1     (PtIn1) = xs     (ii     )
		     y1     (PtIn1) = ys     (ii     )
		  else
		     PtOut1=PtOut1 +1
	    ...
	    if (PtOut1.gt.0) then	      
	    	    print *, 'PTOut1 , total = ',PtOut1, i
		    print *, 'PtZout = ',iz, zs
		    print *, zhat_tmp
		    pause
		    
	       call oa2D(x1,y1,datain1,xhat1,yhat1,PtIn1,
     c                      PtOut1,dataout1,errout1,a,b)

             do ii=1,PtOut1
	          jj = indexes(ii)
	          datains( jj, iz) = dataout1(ii)
	       enddo
	    endif
        enddo
41      continue
...
c	  print*, ' OA - Estimated COV ', i	          
	  call oa(xs,ys,datains,xhat(i),yhat(i),PtIns,1,PtZout,
...
!     OA 2D
!
      subroutine oa2D(x,y,datain,xhat,yhat,PtIn,PtOut,
...
c
	...
	dydx2=(y(n)-y(n-1))/ abs(x(n) - x(n-1))
      ...
      do 30 j=1,ni
        if (xi(j).le.x(1)) then
          ii=1
          yi(j)=y(1) - dydx1 * abs(xi(j) - x(1))
	    
        elseif (xi(j).ge.x(n))then
          yi(j)=y(n)
	    yi(j)=y(n) + dydx2 * abs(xi(j) - x(n))
! flag values that are deeper than the input	    
	    yi(j)=9999
        else
          do 10 i=1,n-1
...
c========================================================	
      subroutine oa(x,y,datain,xhat,yhat,PtIn,PtOut,PtZout,
     c                                      dataout,errout,a,b)
	implicit none
	integer PtIn,PtOut, ii, jj,i,j,iz,PtZout, PtIn_tmp
	real*8 x(PtIn),y(PtIn),xhat(PtOut),yhat(PtOut),datap(PtIn),
     C     dataout(PtOut,PtZout),errout(PtOut),datain(PtIn,PtZout)
	real*8 distb, dista, dist_ref,dist
      real*8 issing,agd,datain_tmp(PtIn),dataout_tmp(PtOut)
c OA variables
      real*8 a,b
	real*8 GD(PtOut,PtIn), DD(PtIn,PtIn), GG(PtOut),
     c      DDinv(PtIn,PtIn) ,m(3),GD_DD(PtOut,PtIn),
...
c remove mean as a plane
	   ...
	   enddo
         ...
         call removeMean(x,y,datain_tmp,PtIn,m,datap)
         call dot(GD_DD,datap,PtOut,PtIn,1,dataout_tmp)   
c  A=GD/DD;
c  dhat=A*d;	
        call addMean(xhat,yhat,dataout_tmp,PtOut,m)
...
c========================================================	
	...
	real*8 x(Pt),y(Pt),datain(Pt),m(3),G(Pt,3),datap(Pt),
     c      dmean(Pt),Gt(3,Pt), b(3,1),Gp(3,3),A(3,3),issing
   ...
   ! setup matrix G
	...
	c=3
      ...
      call dot(Gt,datain ,c,r,1, b)   ! compute b=G'*d
      ...
      if (issing .eq. 1) then
          ...
          call dot(A,b,c,c,1,m)     ! compute m=A*b  
	    call dot(G,m,r,c,1,dmean)     ! compute dmean=G*m  
      endif
