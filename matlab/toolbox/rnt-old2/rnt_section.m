...
%
% FUNCTION [X, Z, SECT, Ipos, Jpos, xcoord, ycoord] = rnt_section(lonr,latr,zr,field,x,y,OPT);
%
...
function [X, Z, SECT, Ipos, Jpos, xg,yg] = rnt_section(lonr,latr,zr,field,x,y,varargin);
...
if OPT.iplot==1
  rnt_contourfill(X,Z,SECT,50);  colorbar 
  hold on
  h_bottom =Z(:,1)';
  x_coord  =X(:,1)';
  xr=x_coord;
  x_coord = [x_coord, x_coord(end), x_coord(1), x_coord(1)];
  h_bottom = [h_bottom, min(h_bottom(:))-10, ...
	min(h_bottom(:))-10, h_bottom(1) ];
