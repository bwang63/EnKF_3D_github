function [z,sc,Cs]=scoord2(h,hice,theta_s,theta_b,Tcline,N,kgrid, ...
                           column,index,plt);
...
%----------------------------------------------------------------------------
  ...
  for k=1:Nlev,
    z0=hc*(sc(k)-Cs(k));
    z(:,k)=z0+Cs(k).*h(index,:)'+hice(index,:)'+ ...
           zeta(index,:)'.*(1.0+(z0+Cs(k).*h(index,:)'))./h(index,:)';
%   z(:,k)=zeta(index,:)'.*(c1+sc(k))+hc.*sc(k)+ ...
%          (h(index,:)'-hc).*Cs(k);
  end,
