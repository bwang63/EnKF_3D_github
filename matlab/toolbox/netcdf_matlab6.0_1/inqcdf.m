...
% INQCDF: an interactive function that returns information about a NetCDF file
%--------------------------------------------------------------------
...
fprintf('');
disp('%% inqcdf is no longer supported - use inqnc from now on %%')
...
% I make mexcdf calls to find the integers that specify the attribute
% types

nc_byte = mexcdf('parameter', 'nc_byte'); %1
nc_char = mexcdf('parameter', 'nc_char'); %2
nc_short = mexcdf('parameter', 'nc_short'); %3
nc_long = mexcdf('parameter', 'nc_long'); %4
nc_float = mexcdf('parameter', 'nc_float'); %5
nc_double = mexcdf('parameter', 'nc_double'); %6
...
if ngatts > 0
   ...
   for i = 0:ngatts-1
     ...
     % display - null characters make a major mess.
	       
      if attype == nc_byte
	 disp(['The following attribute is type byte and may be ' ...
		 'displayed wrongly'])
         s = values;
