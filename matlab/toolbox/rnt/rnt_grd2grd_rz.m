...
function [out,grd,grd1]=rnt_grd2grd_rz(grd,grd1,ctl,time,var,varargin);
...
% raz edit in rgrd_FindInsideIJ section -- this is not giving right coords for V
...
%%% the problem is here %%%%
%%% rgrd_FindInsideIJ does not take into acct diff U,V coords

	[I,J] = rgrd_FindInsideIJ(grd1.lonr,grd1.latr,grd.lonr,grd.latr);
		if gtype == 'u' 
		  if (max(I))>(grd.L)
		    I=I(1:end-1);  
		    disp(['Corrected I max = ',num2str(max(I))]);
		  end	
		
		elseif gtype == 'v'
		  if (max(J))>(grd.M)
		    J=J(1:end-1);
		    disp(['Corrected J max = ',num2str(max(J))]);
		  end
                end
...
if gtype == 'v' 
   ...
   pmapv=rnt_oapmap(grd.lonv(I,J),grd.latv(I,J),grd.maskv(I,J), ...
                    grd1.lonv,grd1.latv,maxpoint); grd1.pmapv=pmapv;	
   end
   if ndim == 3
   [dataout,error]=rnt_oa3d(grd.lonv(I,J),grd.latv(I,J),grd.zrv(I,J,:),field, ...
                grd1.lonv,grd1.latv,grd1.zrv,a,a,grd1.pmapv); 
		    	mask=repmat(grd1.maskv,[1 1 grd1.N]);
			dataout(isnan(mask))=0;  			
   else
   [dataout,error]=rnt_oa2d(grd.lonv(I,J),grd.latv(I,J),field, ...
                grd1.lonv,grd1.latv,a,a,grd1.pmapv);
		    	mask=grd1.maskv;
			dataout(isnan(mask))=0;  			
   end
end
...
if gtype == 'u' 
   ...
   pmapu=rnt_oapmap(grd.lonu(I,J),grd.latu(I,J),grd.masku(I,J), ...
                    grd1.lonu,grd1.latu,maxpoint); grd1.pmapu=pmapu;
   end
  if ndim == 3
  [dataout,error]=rnt_oa3d(grd.lonu(I,J),grd.latu(I,J),grd.zru(I,J,:),field, ...
                grd1.lonu,grd1.latu,grd1.zru,a,a,grd1.pmapu);   			
		    	mask=repmat(grd1.masku,[1 1 grd1.N]);
			dataout(isnan(mask))=0;  			

  else
  [dataout,error]=rnt_oa2d(grd.lonu(I,J),grd.latu(I,J),field, ...
                grd1.lonu,grd1.latu,a,a,grd1.pmapu);
		    	mask=grd1.masku;
			dataout(isnan(mask))=0;  					    
  end
end
...
if gtype == 'r' 
   ...
   pmapr=rnt_oapmap(grd.lonr(I,J),grd.latr(I,J),grd.maskr(I,J), ...
                    grd1.lonr,grd1.latr,maxpoint); grd1.pmapr=pmapr;
   end   						  
   if ndim == 3
