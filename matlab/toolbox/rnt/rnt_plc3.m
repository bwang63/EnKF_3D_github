...
% RNT - E. Di Lorenzo (edl@ucsd.edu)
  ...
  if nargin > 6
     lim = varargin{5};
     xmin=lim.xlim(1); xmax=lim.xlim(2);
     ymin=lim.ylim(1); ymax=lim.ylim(2);
  else
  ...
  if grtype == 'p'
    ...
    rnt_contourfill(lonp.*cmask,latp.*cmask,field.*maskp.*cmask,30)
  ...
  if grtype == 'v'
    ...
    rnt_contourfill(lonv.*cmask,latv.*cmask,field.*maskv.*cmask,30)
    hold on; colormap(rgb);   % colorbar; 
if indc == 1,     [cs,h]=contour(lonv.*cmask,latv.*cmask,field.*maskv.*cmask,numCon,'k');end
  ...
  if grtype == 'u'
    ...
    rnt_contourfill(lonu.*cmask,latu.*cmask,field.*masku.*cmask,30)
    hold on; colormap(rgb);   % colorbar; 
if indc == 1,     [cs,h]=contour(lonu.*cmask,latu.*cmask,field.*masku.*cmask,numCon,'k');end
  end
  
  if grtype == 'r'
    rnt_contourfill(grd.lonr,grd.latr,field.*maskr.*cmask',30);
    hold on; colormap(rgb);   % colorbar; 
