...
function fields=rnt_2s(field,z_r,z)
...
for tile=1:I_TILE*J_TILE
  ...
  tmp=z2scoord_local(field(I,J,:),z_r(I,J,:),z);
  fields(I,J,:)=tmp;
...
function Ts = z2scoord_local(Tn,Sn,Zn)
...
for t=1:time
    Sn1=squeeze(Sn(:,:,:,t));
    Tn1=squeeze(Tn(:,:,:,t));
    Ts1=reshape( rnt_2s_mex_linear(Tn1,size(Tn1),Sn1,size(Sn1),Zn,size(Zn)), [size(Sn1)]);
    Ts(:,:,:,t)=Ts1;
end
