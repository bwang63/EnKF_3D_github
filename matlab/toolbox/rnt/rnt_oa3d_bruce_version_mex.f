...
c note that pmap is a real matrix!!??
	real*8 a,b, pmap(PtOut,PtIns)
c	real*8 a,b
c        integer pmap(PtOut,PtIns)
...
c idbbdc : debug parameter for output.  0 = none, 1 = more, 2 = more, etc.
      ...
      if (idobdc .eq. 1 .and. idbbdc .gt. -1) then
...
c have enough good points to just do the mapping
              call oa(xs,ys,dataouts,xhat(i),yhat(i),ngood,1,1,
     c          dataout_tmp,errout(i),a,b)
...
c revised version from bdc
       call removeMean(x,y,datain_tmp,PtIn,m,datap,xmean,ymean,xl,yl)
...
c      call addMean(xhat,yhat,dataout_tmp,PtOut,m)
      call addMean(xhat,yhat,dataout_tmp,PtOut,m,xmean,ymean,xl,yl)
...
c	remove mean
c========================================================	
      subroutine removeMean(x,y,datain,Pt,m,datap,xmean,ymean,xl,yl)
	implicit none
...
c so if r is 30, 0.1 is a small number to add.
      ...
      if (issing .eq. 1) then
          ...
          call dot(A,b,c,c,1,m)     ! compute m=A*b = (G'*G)^-1 * G'*d
...
c	add mean
c========================================================	
      subroutine addMean(x,y,datain,Pt,m,xmean,ymean,xl,yl)
c      subroutine addMean(x,y,datain,Pt,m)
	implicit none
	integer Pt, ii, jj,i,j,r,c
