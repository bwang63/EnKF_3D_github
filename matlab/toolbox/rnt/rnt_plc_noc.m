...
crgb=[0.15000   0.00000   0.37000
  ...
  if grtype == 'p'
    ...
    pcolor(lonp.*cmask,latp.*cmask,field.*maskp.*cmask)
    colormap(rgb);  shading interp;
    hold on
  ...
  if grtype == 'v'
    ...
    pcolor(lonv.*cmask,latv.*cmask,field.*maskv.*cmask)
    colormap(rgb);  shading interp;
    hold on
if indc == 1,     [cs,h]=contour(lonv.*cmask,latv.*cmask,field.*maskv.*cmask,numCon,'k');end
  ...
  if grtype == 'u'
    ...
    pcolor(lonu.*cmask,latu.*cmask,field.*masku.*cmask)
    colormap(rgb);  shading interp;
    hold on
if indc == 1,     [cs,h]=contour(lonu.*cmask,latu.*cmask,field.*masku.*cmask,numCon,'k');end
  ...
  if grtype == 'r'
    pcolor(lonr.*cmask',latr.*cmask',field.*maskr.*cmask');
    shading interp;
    hold on   ; colormap(rgb);     
    hold on
if indc == 1,     [cs,h]=contour(lonr.*cmask',latr.*cmask',field.*maskr.*cmask',numCon,'k');end
  end
  
  if indl==1
  clabel2(cs,h,'fontsize',14,'color','k','fontname','courier','rotation',0,'LabelSpacing',300);
  end
