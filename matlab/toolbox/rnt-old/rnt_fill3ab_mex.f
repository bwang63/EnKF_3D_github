...
C
      integer*8 plhs(*), prhs(*)
      integer*8 x,y,datain,xhat,yhat,dataout,errout,a,b
	integer*8 z,zhat,c
...
! get address of input      arrays
   ...
   ! set output matrix according to sizes of Sn (3)
	...
	errout = mxGetPr(plhs(2))
     ...
     c          %val(errout),%val(a), %val(b), %val(c))
     ...
     c                 PtIn,PtOut,dataout,errout,a,b,c)
	implicit none
	...
	real*8 distb, dista, dist_ref,dist
      ...
      real*8 xs(PtIn),ys(PtIn),zs(PtIn),datains(PtIn)
c init to zero matricies      
	...
	do i=1,PtOut
	  PtIns=30
	  call selectPt(x,y,z,datain,PtIn,
     c     xs,ys,zs,datains,PtIns,xhat(i),yhat(i),zhat(i))
c         do jj=30,1,-1	   
c	   write(*,*) xs(jj),ys(jj),zs(jj),datains(jj)
c	   enddo

     
	  call oa(xs,ys,zs,datains,xhat(i),yhat(i),zhat(i),
     c             PtIns,1,dataout(i),errout(i),a,b,c)
	   write(*,*) xhat(i),yhat(i),zhat(i), dataout(i)
      enddo
	return
	end
...
c========================================================	
      ...
      subroutine selectPt(x,y,z,datain,PtIn,
     c         xs,ys,zs,datains,PtIns,xhat,yhat,zhat)
	integer PtIn,PtOut, ii, jj,i,j,PtIns
	real*8 x(PtIn),y(PtIn),xhat,yhat,datain(PtIn)
	real*8 z(PtIn),zhat,zs(PtIn)
	real*8 distb, dista,distc, dist_ref,dist(PtIns)
...
c========================================================	
      subroutine oa(x,y,z,datain,xhat,yhat,zhat,
     c          PtIn,PtOut,dataout,errout,a,b,c)
	implicit none
	integer PtIn,PtOut, ii, jj,i,j
	real*8 x(PtIn),y(PtIn),xhat(PtOut),yhat(PtOut),datap(PtIn),
     C        z(PtIn),dataout(PtOut),errout(PtOut),datain(PtIn)
	real*8 distb, dista, dist_ref,dist,zhat(PtOut)
      real*8 issing,agd
c OA variables
      real*8 a,b,c
	real*8 GD(PtOut,PtIn), DD(PtIn,PtIn), GG(PtOut),
     c      DDinv(PtIn,PtIn) ,m(4),GD_DD(PtOut,PtIn),
     c      GDt(PtIn,PtOut)
...
c remove mean as a plane
       call removeMean(x,y,z,datain,PtIn,m,datap)
...
c  dhat=A*d;
      ...
      call addMean(xhat,yhat,zhat,dataout,PtOut,m)
...
c	remove mean
c========================================================	
      subroutine removeMean(x,y,z,datain,Pt,m,datap)
	implicit none
	integer Pt, ii, jj,i,j,r,c
	real*8 x(Pt),y(Pt),z(Pt),datain(Pt),m(4),G(Pt,4),datap(Pt),
     c      dmean(Pt),Gt(4,Pt), b(4,1),Gp(4,4),A(4,4),issing
...
c	add mean
c========================================================	
      subroutine addMean(x,y,z,datain,Pt,m)
	implicit none
	integer Pt, ii, jj,i,j,r,c
	real*8 x(Pt),y(Pt),z(Pt),datain(Pt),m(4),G(Pt,4),
     c      dmean(Pt),Gt(4,Pt), b(4,1),Gp(4,4),A(4,4),issing
