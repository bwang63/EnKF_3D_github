...
section.id = 20; % indicate which cross shore section will be plot
...
% selected section
surface.caxis = {[13 21],[0 1.3], [-2.5 0.7]};
surface.caxis_xtick  = {[13:21],[0:0.2:1.3],[-3:0.5:0.5]};
surface.caxis_xticklabel  = {[13:21],[0:0.2:1.3],[0.001 0.003 0.01 0.03 0.1 0.3 1 3]};

section.caxis = {[13 21],[0 13], [-2 1.2]};
section.caxis_xtick  = {[13:21],[0:2:13],[-2.5:0.5:1.0]};
section.caxis_xticklabel  = {[13:21],[0:2:13],[0.003 0.01 0.03 0.1 0.3 1 3 10]};

%% To plot the spatial distributions in the surface layer
% loop over the variables to be plot
load('redmap.mat'); % load colormap
for icycle = 15:24
for ivar = 1
    ...
    if islog(ivar)
        ...
        pcolor(section.lat, section.dep, section_value.obs); hold on
        [c,h] = contour(section.lat, section.dep, section_value.obs,[1 1]);
        set(h,'color','k')
    end
    shading interp; axis square; box on;
    ...
    if islog(ivar)
        ...
        pcolor(section.lat, section.dep, section_value.for); hold on
        [c,h] = contour(section.lat, section.dep, section_value.obs,[1 1]);
        set(h,'color','k')
        [c,h] = contour(section.lat, section.dep, section_value.for,[1 1]);
    ...
    if islog(ivar)
        ...
        pcolor(section.lat, section.dep, section_value.an); hold on
        [c,h] = contour(section.lat, section.dep, section_value.obs,[1 1]);
        set(h,'color','k')
        [c,h] = contour(section.lat, section.dep, section_value.an,[1 1]);
        set(h,'color','k','linestyle','--')
    end
    shading interp; axis square; box on;
    hold on; plot(section.lat(:,1),section.dep(:,1),'k','linewidth',1); % plot bathymetry along the selected section
