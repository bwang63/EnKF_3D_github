function [kfparams,D] = get_obsposition_full(kfparams,datatime)
% generate/read observations and their locations
%
% by Bin at March 2020
%
% load info from kfparams
index_wet = kfparams.index_wet;  % index for wet points (including whole water column)
n_wet = length(index_wet); % total number of wet grid 
...
mask_wet = kfparams.domainmask; % one layer only, 1 for wet and nan for dry grid cells
mask_wet_l = ~isnan(mask_wet); % logical matrix, 1 for wet and 0 for dry grid cells
index_wet_h = find(mask_wet_l); % vector, the index of wet grid cells 
...
n_obs = zeros(1,n_provenance); % number of observations for each observation source (e.g. SST, Targo)
...
obs_time = nc_varget(kfparams.obsfile,'obs_time') + kfparams.refdate;
obs_provenanceID = nc_varget(kfparams.obsfile,'obs_provenance');

for i_provenance = 1:n_provenance
    Tindex = find(obs_time == datatime & obs_provenanceID == provenanceID(i_provenance));
    if isempty(Tindex)
        % counting the number of observations for each provenance
        n_obs(i_provenance) = 0;
    else
        if any(unique(diff(Tindex))~=1)
            ...
            % counting the number of observations for each provenance
             n_obs(i_provenance) = numel(obs_value);
...
kfparams.index_flatstate2obs_l = false(numel(index_state),n_provenance);

icount = 0;
for i_provenance = 1:n_provenance
    for i_obs = 1:n_obs(i_provenance)
        ...
        kfparams.index_flatstate2obs_l(obspos_ind,i_provenance) = 1; 
    end
end
% D = rmfield(D,'Lgrid_3D');
% only save horizontal coefficient in those grid cells that are impacted by observations
