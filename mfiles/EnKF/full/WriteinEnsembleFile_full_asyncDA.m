function WriteinEnsembleFile_full_asyncDA(kfparams,yfor,yana,D,assimstep,i_assimvar)
%
...
nc = netcdf(fileout,'w');  % nc file is created in rnc_CreateEnsembleFile_v1, called by CreateEnsembleFile
...
if i_assimvar ==1  % only need to save the observation once
    ...
    for idate = 1: kfparams.nobsdates  % loop for observed dates
        for iobs = 1:size(index_obs,2)  % loop for observation provenance
            index = index_obs(:,iobs,idate);

            if idate==1
                if iobs==1
                    i1 = 1;
                else
                    i1 = sum(sum(n_obs(1,1:iobs-1)))+1;
                end
                i2 = sum(sum(n_obs(idate,1:iobs)));
            else
                if iobs==1
                    i1 = sum(sum(n_obs(1:idate-1,:)))+1;
                else
                    i1 = sum(sum(n_obs(1:idate-1,:))) + sum(sum(n_obs(idate,1:iobs-1)))+1;
                end
                i2 = sum(sum(n_obs(1:idate-1,:))) + sum(sum(n_obs(idate,1:iobs)));
            end
            ...
            nc{str}(:,:,:,idate) = shiftdim(Dall,2); % shift the depth dimension from the last to first position
        end
    end
